...
export default function Minimap() {
    ...
    // ── Tooltip helper ──
    const renderTooltip = (px: number, py: number, lines: { text: string; color: string }[]) => {
        const tw = 170;
        const lineH = 16;
    ...
    return (
        <div ref={wrapperRef} className={styles.wrapper}>
            ...
            >
                ...
                {/* ── Search area donut — hidden during autonomous mode ── */}
                        ...
                        >
                            ...
                            {isHovered && (
                                <g pointerEvents="none">
                                    ...
                                    {renderTooltip(px, py, [
                                        {
                                            text: `x: ${Math.round(ping.rover_position.x)}, y: ${Math.round(ping.rover_position.y)}`,
                                            color: color,
                                        },
                                        {
                                            text: `${Math.round(ping.rssi)} dBm · ${ping.signal_category.replace("_", " ")}`,
                                            color: "#aaa",
                                        },
                ...
                {isRoverHovered &&
                    renderTooltip(roverPosition.x, -roverPosition.y - 30, [
                        {
                            text: `x: ${Math.round(roverPosition.x)}, y: ${Math.round(roverPosition.y)}`,
                            color: "#6ee7b7",
                        },
                    ])}
                        ...
                        <g key={label}>
                            ...
                            {hoveredEva === label &&
                                renderTooltip(imu.posx, -imu.posy - 24, [
                                    {
                                        text: `x: ${Math.round(imu.posx)}, y: ${Math.round(imu.posy)}`,
                                        color: color,
                                    },
                                ])}
                        </g>
