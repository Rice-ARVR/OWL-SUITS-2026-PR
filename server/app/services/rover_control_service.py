...
async def send_rover_command(payload: dict) -> None:
    ...
    if "cabin_heating" in payload:
        await asyncio.to_thread(
            tss_client.send_cabin_heating, float(payload["cabin_heating"])
        )
    if "cabin_cooling" in payload:
        await asyncio.to_thread(
            tss_client.send_cabin_cooling, float(payload["cabin_cooling"])
        )
    if "headlights" in payload:
        await asyncio.to_thread(
            tss_client.send_headlights, float(payload["headlights"])
        )
